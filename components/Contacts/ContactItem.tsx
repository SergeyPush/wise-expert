...
const ContactItem = ({ item: { Icon, text, link } }: ContactItemInterface) => {
  return (
    <Link
      href={link}
      target="_blank"
      className="flex items-center gap-2 p-3 rounded-xl bg-color-light-gray hover:bg-color-light-blue transition-colors duration-200 group min-w-0"
    >
      ...
      <span className="text-base xl:text-[15px] font-medium text-color-black whitespace-nowrap">{text}</span>
    </Link>
  );
};
